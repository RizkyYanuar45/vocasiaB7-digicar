...
exports.approveAndProcessPayment = async (req, res) => {
  ...
  try {
    ...
    await sendEmail(userEmail, 'Approval Notification', `Your order has been approved! Please complete your payment at the following link: ${redirectUrl.token.redirect_url}`);
...
exports.checkPaymentOrder = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Status pembayaran berhasil diperbarui!', order });
  } catch (error) {
    console.error(error);
    res.status(500).json({
...
exports.deleteApprovedOrder = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Order berhasil dihapus dan status mobil diperbarui!' });
  } catch (error) {
    console.error(error);
    res.status(500).json({
...
exports.declineOrder = async (req, res) => {
  ...
  try {
    ...
    const userEmail = order.contact;
    await sendEmail(userEmail, 'Order Declined', 'Your order has been declined. If you have any questions, please contact us.');

    await Order.findByIdAndDelete(orderId);

    res.status(200).json({ message: 'Order has been declined and deleted' });
  } catch (err) {
    res.status(500).json({ message: 'Error declining order', error: err.message });
  }
};

            ...
            {products.map((product) => (
              <SwiperSlide key={product._id}>
                ...
                >
                  ...
                  />
                  <h3
                    id={`product-${product._id}-heading`}
                    className="text-2xl font-bold text-black-950 mb-2"
                  >
                    {product.name}
                  </h3>
                  <p className="text-lg text-black-950 mb-4">{product.plate}</p>
                  <div className="text-2xl font-bold mb-6">
                    Rp. {product.pricePerDay.toLocaleString("id-ID")}/ 1 Hari
                  </div>
                  <div className="flex gap-8">
                    <a
                      href="#"
                      className="w-1/2 py-2 text-md rounded-full text-center bg-night-shadz-800 hover:bg-night-shadz-900 text-white-50"
                      onClick={() => handleOpenDetail(product)}
                    >
                      Detail
                    </a>
                    <Link
                      to={`/catalog/order/${product._id}`}
                      className="w-1/2 py-2 text-md rounded-full text-center bg-night-shadz-800 hover:bg-night-shadz-900 text-white-50"
                    >
                      Pesan Sekarang
                    </Link>

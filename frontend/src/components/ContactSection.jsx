...
export default function ContactSection() {
  ...
  return (
    <section className="flex flex-col items-center">
      <h2 className="mt-11 text-5xl font-bold text-center border border-solid border-stone-50 text-stone-800 max-md:mt-10 max-md:text-4xl animate__animated animate__slideInUp">Temukan Kami</h2>
      <div className="shrink-0 mt-8 max-w-full bg-rose-800 border-rose-800 border-solid border-[5px] h-[5px] w-[273px]" />
      <p className="mt-11 ml-5 text-2xl font-bold text-center text-black max-md:mt-10 max-md:text-xl">Anda mempunyai pertanyaan? Silakan hubungi kami melalui</p>
      ...
      <div className="flex flex-col md:flex-row gap-5 justify-between mt-20 max-w-full w-full max-md:mt-10 max-md:pl-4">
        <div className="flex flex-col self-start w-full md:w-1/2">
          {contactInfo.map((info, index) => (
            <div key={index} className={`flex gap-10 ${index > 0 ? 'mt-1' : ''} max-md:mr-2.5 max-md:gap-5`} data-aos="fade-right">
              <img loading="lazy" src={info.icon} className="object-contain shrink-0 self-start mt-2.5 w-10 aspect-[1.08]" alt={`Contact icon ${index + 1}`} />
              <div className="flex flex-col grow shrink-0 text-black basis-0 w-fit">
                <div className="self-start text-2xl font-bold max-md:text-xl max-md:text-left">{info.phone}</div>
                <div className="mt-4 text-lg max-md:text-base max-md:text-left">{info.hours}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col text-2xl font-bold text-center text-black whitespace-nowrap w-full md:w-1/2">
          {socialLinks.map((social, index) => (
            <a key={index} href={social.link} target="_blank" rel="noopener noreferrer" className={`flex gap-6 ${index > 0 ? 'mt-7' : ''} self-start`} data-aos="fade-left">
              <img loading="lazy" src={social.icon} className={`object-contain shrink-0 aspect-[1.23] w-[43px] ${social.rounded ? 'rounded-[50px]' : ''}`} alt={`${social.name} icon`} />

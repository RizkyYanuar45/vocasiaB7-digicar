...
export const Reset = () => {
  ...
  const handleRequestResetCode = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('https://v1.digicar.my.id/api/auth/request-password-reset', {
        method: 'POST',
        headers: {
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('https://v1.digicar.my.id/api/auth/reset-password', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        ...
        <form onSubmit={step === 'request' ? handleRequestResetCode : handleResetPassword} className="flex flex-col justify-center items-center min-h-[calc(100vh-8rem)] lg:min-h-[calc(100vh-10rem)] overflow-y-auto px-4">
          ...
          {step === 'reset' && (
            <>
              ...
              <button type="submit" className="bg-primary text-white-50 p-3 mt-6 rounded-lg w-1/2" disabled={loading}>

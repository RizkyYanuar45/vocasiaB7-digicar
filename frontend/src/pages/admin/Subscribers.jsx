...
export const Subscribers = () => {
  ...
  useEffect(() => {
    const fetchSubscribers = async () => {
      try {
        const response = await axios.get('https://v1.digicar.my.id/api/subscriber', {
          headers: {
  ...
  const deleteSubscriber = async (email) => {
    try {
      await axios.delete(`https://v1.digicar.my.id/api/subscriber/${email}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      setSubscribers((prevSubscribers) => prevSubscribers.filter((subscriber) => subscriber.email !== email));
    } catch (error) {
              ...
              <tbody>
                {filteredSubscribers.map((subscriber) => (
                  <tr key={subscriber._id} className="border-b border-black">
                    ...
                    <td className="px-2 py-1">{formatDate(subscriber.createdAt)}</td>
                    <td className="px-2 py-1">
                      <div onClick={() => handleDelete(subscriber.email)} className="flex items-center bg-red-700 justify-center text-white-50 p-1 rounded-xl cursor-pointer">

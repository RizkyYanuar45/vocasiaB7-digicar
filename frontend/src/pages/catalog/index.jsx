      ...
      <div className="bg-gradient-to-r from-cinderella-100 to-white-50">
        <div className="container w-full py-24 flex ">
          <div className="flex flex-col w-full md:w-7/12 animate__animated animate__slideInLeft">
            <h1 className="text-4xl text-night-shadz-700 font-bold">Pilih Armada Anda..</h1>
            <p className="text-lg font-base my-3 text-scorpion-700">
              Kami hanya menyediakan unit mobil berkualitas yang siap untuk melakukan perjalanan dalam dan luar kota di seluruh Indonesia. Berikut ini adalah beberapa pilihan kategori mobil yang bisa Anda sewa sesuai kebutuhan Anda.
            </p>
            <div className="bg-white-50 rounded w-7/12 flex p-2">
              <input type="text" className="w-8/12 text-black-950 px-5" placeholder="Masukkan Pilihan Anda" value={searchTerm} onChange={handleSearchChange} />
              <button className="bg-night-shadz-700 text-lg font-semibold text-white-50 w-4/12 rounded p-2">Cari</button>
            </div>
            <div className="flex gap-4 mt-4 items-center">
              <button onClick={handleToggleNewYear} className={`p-2 rounded-lg ${isNewYearActive ? 'bg-primary text-white-50 font-main ' : 'bg-transparent outline outline-1 outline-primary font-main'}`}>
                Tahun Baru
              </button>
          ...
          <div className="hidden md:block md:w-5/12 relative p-5 animate__animated animate__slideInRight">
            <img src="/catalog.png" alt="catalog" className="w-full h-full bg-contain" />
          </div>
        </div>
      </div>
      <Container>
          ...
          {loading ? (
            ...
            filteredCars.map((car, idx) => (
              <CardCatalog
                ...
                carId={car._id}
                number={idx}
                image={`https://v1.digicar.my.id/${car.image.replace(/\\/g, '/')}`}
                name={car.name}
